...
def encoder_text(text_encoder,prompt,negative_prompt,save_emb,target_length=640):
    ...
    # Free the text encoder from memory before the large DiT model loads
    del text_encoder
...
def read_lat_emb(prefix, positive, negative,device):
    if prefix =="embeds":
        ...
        audio_lat=audio_latents["samples"].to(device,torch.bfloat16)
        print(f"audio shape: {audio_lat.shape}")
...
def save_lat_emb(save_prefix,data1,data2,mode=""):
    ...
    if os.path.exists(default_data1_path):
