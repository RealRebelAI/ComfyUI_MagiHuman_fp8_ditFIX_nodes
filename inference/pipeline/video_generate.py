...
class MagiEvaluator:
    def __init__(
        ...
        print_mem_info_rank_0("After init MagiEvaluator")
    ...
    def evaluate(
        ...
        latent_video = torch.randn(
            1, self.z_dim, latent_length, br_latent_height, br_latent_width,
        ...
        if sr_width is not None and sr_height is not None and self.sr_model is not None:
            ...
            latent_video = torch.nn.functional.interpolate(
                br_latent_video,
                size=(params["latent_length"], sr_latent_height, sr_latent_width),
                mode="trilinear",
                align_corners=True,
            )
            ...
            print_rank_0("Flushing base model blocks before SR pass...")
            if self.model is not None and hasattr(self.model, 'block_gpu_manager'):
                self.model.block_gpu_manager.unload_all_blocks_to_cpu()
            gc.collect()
            torch.cuda.empty_cache()
            if torch.cuda.is_available():
                torch.cuda.ipc_collect()
            print_mem_info_rank_0("VRAM after base model flush")
            ...
            br_latent_audio = (
                torch.randn_like(br_latent_audio, device=br_latent_audio.device)
                * self.config.sr_audio_noise_scale
                + br_latent_audio * (1 - self.config.sr_audio_noise_scale)
            )
    ...
    def evaluate_with_latent(
        ...
        if offload:
            ...
            if use_sr_model and self.sr_model is not None:
                gpu_manager.setup_for_inference(self.sr_model)
            else:
                model_to_use = self.model if self.model is not None else self.sr_model
                if model_to_use is not None:
                    gpu_manager.setup_for_inference(model_to_use)
                else:
                    gpu_manager = None
        else:
            gpu_manager = None
